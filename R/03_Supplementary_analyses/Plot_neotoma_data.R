...
(p1 <-
    ...
    ggplot2::geom_tile(
      ...
      colour = NA,
      alpha = scales::rescale(dat_geo$N_bin, to = c(0.5, 1))
    ) + 
    ggplot2::borders(
      fill = NA,
      colour = "gray30",
    ...
    ggplot2::scale_fill_viridis_d(
      direction = 1,
      option = "C",
      breaks = dat_geo$N_bin %>% 
        unique() %>% 
        sort(),
      labels = dat_geo$N_bin %>% 
        unique() %>% 
        sort() %>% 
        as.character(),
...
ggplot2::ggsave(
  plot = p1,
  filename = paste0(current_dir, "/Outputs/Figures/Supplementary/Neotoma_data_geo-",current_date, ".pdf"),
  width = image_width,
  height = image_height,
  units = image_units,
  dpi = image_dpi)
...
ggplot2::ggsave(
  plot = p2,
  filename = paste0(current_dir, "/Outputs/Figures/Supplementary/Neotoma_data_age-",current_date, ".pdf"),
  width = image_width,
  height = image_height,
  units = image_units,
  dpi = image_dpi)
